package io.misskey.mc.core.modules.ranking
...
object RankingModule : ModuleBase() {
     ...
     */
    fun renderAll() {
        for (hologram in HologramsAPI.getHolograms(IomcCorePlugin.instance)) {
            hologram.delete()
        }
        for (ranks in getAll()) {
            val loc = ranks.getHologramLocation() ?: return
            val isHidden = ranks.getHologramHidden()
            val holo = HologramsAPI.createHologram(IomcCorePlugin.instance, loc)
